...
const VERSION_URL = `https://github.com/${UPDATE_REPO}/releases/latest/download/version.json`;
const APK_URL = `https://github.com/${UPDATE_REPO}/releases/latest/download/blackstar-player.apk`;
...
/** Returns update info when a newer version is available on GitHub, else null. */
export async function checkForUpdate(): Promise<UpdateInfo | null> {
  ...
  try {
    ...
    if (!version || cmpVersion(version, APP_VERSION) <= 0) return null;
